...
void generate_hann_window(float32_t *output) {
	for(int i = 0; i < FFT_SIZE; i++) {
		float32_t value = 0.5f * (1.0f - arm_cos_f32((2 * PI * (float32_t)i) / (float32_t)(FFT_SIZE - 1)));
		output[2 * i] = value; //window value for I
		output[2  * i + 1] = value; //window value for Q
	}
}
